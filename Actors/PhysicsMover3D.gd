...
func add_acceleration(added_acc: Vector3):
	acceleration += added_acc
	
func add_plane_acceleration(added_acc: Vector2):
...
#	velocity = move_and_slide_with_snap(velocity, snap_vector, Vector3.UP, stop_on_slopes, max_slides, floor_max_angle)
	move_and_slide() # velocity, Vector3.UP, stop_on_slopes, max_slides, floor_max_angle)
	var collision = get_last_slide_collision()

	if collision != null:
		var height = capsule_height + (collision.get_position().y - global_transform.origin.y)
